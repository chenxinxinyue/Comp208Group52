...
//            if (i != localPlayerIDSeq && !playerIDList.get(i).equals("")) {
//                Node newPlayer = createPlayer(i + 1);
//                newPlayer.setName("remotePlayer" + i);
...
//            }
//
//            playerRootNode.getChild(i).setLocalTranslation(playerLocationList[i]);
//        }
        int playerCounter = 0;
...
//                    localPlayer.setLocalTranslation(new Vector3f(2f, 2f, 0.001f));
                    ...
                    newPlayer.setName("remotePlayer" + (playerCounter + 1));
                    playerRootNode.attachChild(newPlayer);
                }
...
//        playerGeom.move(0, 0, 0.01f);
        ...
        newPlayer.attachChild(movementDirectionPointers);
        ...
        newPlayer.attachChild(projectileDirectionPointers);
...
//        throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
    ...
    public void update(float tpf) {
        ...
        printWriter.println("GameStatusRequest," + lobbyID);
...
//        if (clientSocket.isConnected() && !clientSocket.isClosed() && inputScanner.hasNextLine()) {
//            String receivedMessage = inputScanner.nextLine();
//            System.out.println("Received [" + receivedMessage + "] from server.");
//
        ...
        if (loopTimer < 0 && playerAction == actionCase.SHOOT) {
            ...
            for (Spatial player : playerRootNode.getChildren()) {
                if (player instanceof Node) {
                    ...
                    Spatial movementDirectionPointers = playerNode.getChild("movement direction pointers");
                    Spatial projectileDirectionPointers = playerNode.getChild("projectile direction pointers");
            ...
            for (Spatial projectile : projectileRootNode.getChildren()) {
                ...
                projectile.setLocalTranslation(Math.round(projectileLocation.x), Math.round(projectileLocation.y), 0);
            ...
            for (Spatial player : playerRootNode.getChildren()) {
                switch (selectedMovementDirection[movementAnimationloopCounter]) {
                    case NONE:
                        break;
                    case UP:
                        localPlayer.move(0, newPositionIncrement - lastPositionIncrement, 0);
                        break;
                    case LEFT:
                        localPlayer.move(-newPositionIncrement + lastPositionIncrement, 0, 0);
                        break;
                    case DOWN:
                        localPlayer.move(0, -newPositionIncrement + lastPositionIncrement, 0);
                        break;
        ...
        for (Spatial player : playerRootNode.getChildren()) {
            ...
            if (!newLocalPlayProjectileCreated[projectileCreationLoopCounter] && playerAction == actionCase.SHOOT && loopTimer <= 1.0f) {
                ...
                if (selectedProjectileDirection[projectileCreationLoopCounter] != ProjectileDirectionCase.NONE) {
                    ...
                    createProjectile(playerPosX, playerPosY, selectedProjectileDirection[projectileCreationLoopCounter], newProjectileTimeToLive);
                }
            }
...
//    };
    private void createObstacle(int x, int y) {
        ...
        Texture obsTex = assetManager.loadTexture("Textures/tileSrc3.png");
    ...
    private void updateMovementDirectionPointers(Geometry movementDirectionPointers, MovementDirectionCase dir) {
        ...
        switch (dir) {
            case UP:
                newMovementDirectionPointersTex = assetManager.loadTexture("Textures/moveUp.png");
                break;
            case DOWN:
                newMovementDirectionPointersTex = assetManager.loadTexture("Textures/moveDown.png");
                break;
            case LEFT:
                newMovementDirectionPointersTex = assetManager.loadTexture("Textures/moveLeft.png");
                break;
            case RIGHT:
                newMovementDirectionPointersTex = assetManager.loadTexture("Textures/moveRight.png");
                break;
            case NONE:
                break;
            case UNAVAILABLE:
    ...
    private void updateProjectileDirectionPointers(Geometry projectileDirectionPointers, ProjectileDirectionCase dir) {
        ...
        switch (dir) {
            case UP:
                newprojectileDirectionPointersTex = assetManager.loadTexture("Textures/shootU.png");
                break;
            case DOWN:
                newprojectileDirectionPointersTex = assetManager.loadTexture("Textures/shootD.png");
                break;
            case LEFT:
                newprojectileDirectionPointersTex = assetManager.loadTexture("Textures/shootL.png");
                break;
            case RIGHT:
                newprojectileDirectionPointersTex = assetManager.loadTexture("Textures/shootR.png");
                break;
            case UPLEFT:
                newprojectileDirectionPointersTex = assetManager.loadTexture("Textures/shootUL.png");
                break;
            case DOWNLEFT:
                newprojectileDirectionPointersTex = assetManager.loadTexture("Textures/shootDL.png");
                break;
            case UPRIGHT:
                newprojectileDirectionPointersTex = assetManager.loadTexture("Textures/shootUR.png");
                break;
            case DOWNRIGHT:
                newprojectileDirectionPointersTex = assetManager.loadTexture("Textures/shootDR.png");
                break;
            case NONE:
                break;
            case UNAVAILABLE:
    ...
    private void createProjectile(int playerPosX, int playerPosY, ProjectileDirectionCase dir, int ttl) {
//        System.out.println(playerPosX + ", " + playerPosY);
        Vector3f velocity = new Vector3f();

        Geometry newProj = new Geometry("projectileGeom", new Quad(1, 1));
        ...
        switch (dir) {
            case DOWN:
                newProjMat.setTexture("ColorMap", assetManager.loadTexture("Textures/projectile1Down.png"));
                velocity = new Vector3f(0, -1, 0);
                break;
            case UP:
                newProjMat.setTexture("ColorMap", assetManager.loadTexture("Textures/projectile1Up.png"));
                velocity = new Vector3f(0, 1, 0);
                break;
            case LEFT:
                newProjMat.setTexture("ColorMap", assetManager.loadTexture("Textures/projectile1Left.png"));
                velocity = new Vector3f(-1, 0, 0);
                break;
            case UPLEFT:
                newProjMat.setTexture("ColorMap", assetManager.loadTexture("Textures/projectile2UL.png"));
                velocity = new Vector3f(-1, 1, 0);
                break;
            case DOWNRIGHT:
                newProjMat.setTexture("ColorMap", assetManager.loadTexture("Textures/projectile2DR.png"));
                velocity = new Vector3f(1, -1, 0);
                break;
            case DOWNLEFT:
                newProjMat.setTexture("ColorMap", assetManager.loadTexture("Textures/projectile2DL.png"));
                velocity = new Vector3f(-1, -1, 0);
                break;
            case RIGHT:
                newProjMat.setTexture("ColorMap", assetManager.loadTexture("Textures/projectile1Right.png"));
                velocity = new Vector3f(1, 0, 0);
                break;
            case UPRIGHT:
                newProjMat.setTexture("ColorMap", assetManager.loadTexture("Textures/projectile2UR.png"));
                velocity = new Vector3f(1, 1, 0);
                break;
        }
    ...
    private void changeDiagonalProjectile(projectionReflectionCase rType, Material newProjMat, Spatial projectile, Vector3f velocity) {
        if (rType == projectionReflectionCase.DIAGONAL_CASE_XY) {
            ...
            newProjMat.setTexture("ColorMap", assetManager.loadTexture(getDiagonalProjectileTexturePath(newVel)));
    ...
    private String getDiagonalProjectileTexturePath(Vector3f vel) {
        String name = "Textures/projectile2UL.png";

        if (vel.equals(new Vector3f(-1, -1, 0))) {
            name = "Textures/projectile2DL.png";
        }
        if (vel.equals(new Vector3f(1, -1, 0))) {
            name = "Textures/projectile2DR.png";
        }
        if (vel.equals(new Vector3f(1, 1, 0))) {
            name = "Textures/projectile2UR.png";
        }
        if (vel.equals(new Vector3f(-1, 1, 0))) {
            name = "Textures/projectile2UL.png";
        }
